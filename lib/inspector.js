...
export default function Inspector(relay, opts={})
{
  ...
  console.log(relay, 'options', opts)
...
//  if(benchmark) //debug.info(url, subid, this.result.latency.start)
  ...
  this.result.timeout[which] = setTimeout(() => {
...
Inspector.prototype.try_complete = function() {
  ...
  let connect = typeof this.result.check.connect !== 'object', //check null
      read = typeof this.result.check.read !== 'object' || !this.opts.checkRead,
      write = typeof this.result.check.write !== 'object' || !this.opts.checkWrite,
      latency = typeof this.result.check.latency !== 'object' || !this.opts.checkLatency

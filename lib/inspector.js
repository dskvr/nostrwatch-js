...
export default function Inspector(relay, opts={})
{
  ...
  this.relay = (relay instanceof(Inspector)) ? relay : new Relay(relay)
...
Inspector.prototype.on_notice = function(notice) {
  // if(this.opts.debug) console.log(this.relay.url, "on_notice")
...
Inspector.prototype.observe = function() {
  ...
  if(this.result.count.read > 1)
    this.result.observations.push(new Observation('FILTER_LIMIT_IGNORED', 'The relay returned more events than were asked for', 'sub:filter:limit'))
}
